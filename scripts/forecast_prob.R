...
departements <- c('Artibonite', 'Centre', 'Grande_Anse', 'Nippes', 'Nord',
...
for(dp in departements) {
  print(dp)
  load(file = sprintf("output/Simulations/Haiti_OCV_Projection-%s-%i-%s.rda", dp, nsim, scenario))
  if (dp == 'Artibonite') {
    df <- projec %>% filter(sim != 'data', time > t_vacc_start) %>% select(time, sim, I)
    colnames(df)[names(df) == "I"] <- dp
    
  }else{
    dat <- projec %>% filter(sim != 'data', time > 2019) %>% select(time, sim, I)
...
for (s in 1:nsim) {
  ...
  if (sum(df_s %>% filter(time > t_vacc_start + 3 - 6/12) 
  ...
  if (sum(df_s %>% filter(time > t_vacc_start + 5 - 6/12) 
  ...
  if (sum(df_s %>% filter(time > t_vacc_start + 10 - 6/12) 
...
print(paste0('10 years T1 : ', acc_10y_thresh2/nsim * 100))
