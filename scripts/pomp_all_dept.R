...
param_rates_in_days_names <-
  c("mu", "alpha", "gammaI", "gammaA", "rhoA") #muB
...
fixed_input_parameters <-
  as_vector(input_parameters[map_lgl(names(input_parameters), ~ . %in% param_fixed_names)])


# set fixed process parameters
param_proc_fixed <-
  set_names(seq_along(param_proc_fixed_names) * 0, param_proc_fixed_names)
param_proc_fixed[names(fixed_input_parameters)] <-
  fixed_input_parameters
...
nb_doses_alt_year <-
  unlist(flatten(input_parameters["nb_doses_alt_year"]))
t_vacc_start_alt  <-
  unlist(flatten(input_parameters["t_vacc_start_alt"]))
t_vacc_end_alt <-
  unlist(flatten(input_parameters["t_vacc_end_alt"]))
...
param_fixed <-
  set_names(seq_along(param_fixed_names) * 0, param_fixed_names)
...
param_est <-
  set_names(seq_along(param_est_names) * 0, param_est_names)
...
sirb_cholera <- pomp(
  ...
  globals = str_c(
    sprintf(
      "double t_vacc_start_alt = %f; double t_vacc_end_alt = %f;",
      t_vacc_start_alt,
      t_vacc_end_alt
    ),
